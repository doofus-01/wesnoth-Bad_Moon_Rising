...
--		maximum_width = 800,
	...
	uor_noc = wesnoth.units.find_on_recall( { side = "1", {"filter_wml", {{"variables", { on_call = "no"} } }}})
	    
	local ioc = wml.variables["total_on_call"]
	local iupkeep = wml.variables["on_call_upkeep"]
	if iupkeep then
	else
	iupkeep = 0
	end        			
    ...
    local widget_handle = self:find('the_headcount')
    ...
    local function initial_toggle_on_call()
	    for i in ipairs(uor) do
            ...
            widget_handle = self:find("the_on_list", i , "the_on_call_button")
            widget_handle.selected = uor[i].variables.on_call
	    end
	    for i in ipairs(uor_noc) do
            ...
            widget_handle.selected = uor_noc[i].variables.on_call
	    end
	end
...
-- to display traits
	    ...
	    if unit then
	      local uor_mods = wml.get_child(unit.__cfg, "modifications")
	      local c_i = 1
	      for trait in wml.child_range( uor_mods, "trait") do
          ...
          widget_handle = self:find('the_unit_name')
          widget_handle.use_markup = true
          widget_handle.label = string.format("<span size='large' underline='single'>%s</span>", unit.__cfg.name)
          ...
          widget_handle = self:find('the_unit_type')
          widget_handle.use_markup = true
          widget_handle.label = string.format("<span style='oblique' color='#aabbcf'>%s</span>", unit.__cfg.type)      
          ...
          widget_handle = self:find('the_unit_traits')
          widget_handle.use_markup = true
          widget_handle.label = string.format("<span variant='smallcaps' color='#bfa7a7'> %s \n %s</span>", traits_strings[1], traits_strings[2])      
          -- wesnoth.set_dialog_value(string.format("%s~SCALE(200,200)", unit.__cfg.profile) , "the_unit_profile")
          widget_handle = self:find('the_unit_profile')
          widget_handle.label = string.format("%s~SCALE(200,200)", unit.__cfg.profile)
-- to make a blank list, otherwise we are dominated by the first unit shown, I think.  This can probably be improved.  Seems redundant
          local k = 1
          while k < 7 do
            -- wesnoth.set_dialog_value(string.format("misc/blank-hex.png~SCALE(30,30)"), "the_gearlist", k, "the_gearlist_icon")
            widget_handle = self:find("the_gearlist", k, "the_gearlist_icon")
            widget_handle.label = string.format("misc/blank-hex.png~SCALE(30,30)")
            k = k + 1
          end          
...
--  first we black them out, to reserve space and to erase old images
          ...
          g_i = 1
          local uor_gear = wml.get_child(unit.__cfg, "variables")
          for gear in wml.child_range( uor_gear , "gear") do
		    -- wesnoth.set_dialog_value(string.format("%s~SCALE(30,30)", gear.image), "the_gearlist", g_i, "the_gearlist_icon")                        
            widget_handle = self:find("the_gearlist", g_i, "the_gearlist_icon")
            widget_handle.label = string.format("%s~SCALE(30,30)", gear.image)
		    g_i = g_i + 1
    ...
    local function select_on()
        ...
        widget_handle = self:find('the_on_list')
    ...
    widget_handle = self:find("the_off_list")
    widget_handle.callback = select_off
    
    -- wesnoth.set_dialog_callback(select_on, "the_on_list")
    widget_handle = self:find("the_on_list")
    widget_handle.callback = select_on
...
--            while k < 7 do
    ...
    widget_handle = self:find("the_on_list", k, "the_icon")
    widget_handle.label = string.format("misc/blank-hex.png~SCALE(72,72)")
    -- wesnoth.set_dialog_value("Empty List", "the_on_list", k, "the_label")
    widget_handle = self:find("the_on_list", k, "the_label")
    widget_handle.label = string.format("Empty List")
    -- wesnoth.set_dialog_value("-", "the_on_list", k, "the_XP")
    widget_handle = self:find("the_on_list", k, "the_XP")
    widget_handle.label = string.format("-")
    -- wesnoth.set_dialog_value("-", "the_on_list", k, "the_HP") 
    widget_handle = self:find("the_on_list", k, "the_HP")
    widget_handle.label = string.format("-")
    -- wesnoth.set_dialog_value(string.format("misc/blank-hex.png~SCALE(72,72)"), "the_off_list", k, "the_icon")
    widget_handle = self:find("the_off_list", k, "the_icon")
    widget_handle.label = string.format("misc/blank-hex.png~SCALE(72,72)")
    -- wesnoth.set_dialog_value("Empty List", "the_off_list", k, "the_label")
    widget_handle = self:find("the_off_list", k, "the_label")
    widget_handle.label = string.format("Empty List")
    -- wesnoth.set_dialog_value("-", "the_off_list", k, "the_XP")
    widget_handle = self:find("the_off_list", k, "the_XP")
    widget_handle.label = string.format("-")
    -- wesnoth.set_dialog_value("", "the_off_list", k, "the_HP") 
    widget_handle = self:find("the_off_list", k, "the_HP")
    widget_handle.label = string.format("-")
--                k = k + 1
--            end          
	for i in ipairs(uor) do
        ...
        widget_handle = self:find("the_on_list", i, "the_label")
        widget_handle.label = string.format("<span size='small'> %s </span>" , ut.name)
        widget_handle.use_markup = true
        ...
        widget_handle = self:find("the_on_list", i, "the_XP")
        widget_handle.label = string.format("<span size='x-small' "..xp_color..">XP: %s</span>", ut.experience)
        widget_handle.use_markup = true
        ...
        widget_handle.label = string.format("%s~RC(magenta>red)", ut.image)
	end
-- make the off-duty text and icons grey
	for i in ipairs(uor_noc) do
        ...
        widget_handle.label = string.format("<span size='small' color='#777777' > %s </span>" , ut.name)
        widget_handle.use_markup = true
        ...
        widget_handle = self:find("the_off_list", i, "the_XP")
        widget_handle.label = string.format("<span size='x-small' color='#777777' >XP: %s</span>", ut.experience)
        widget_handle.use_markup = true
        ...
        widget_handle.label = string.format("%s~GS()", ut.image)
    end                    	                      
...
-- ... and, if dialog was dismissed with OK or Update buttons, then write thr button values to the units (of both lists)
       if r == -1 or r == 3 then
		 ...
		 local iloyal = "no"
                 ...
                 ioc = ioc + 1
	         local uor_mods = wml.get_child(uor[i].__cfg, "modifications")
	         for trait in wml.child_range( uor_mods, "trait") do
		 ...
		 local iloyal = "no"
                 ...
                 ioc = ioc + 1
	         local uor_mods = wml.get_child(uor_noc[i].__cfg, "modifications")
	         for trait in wml.child_range( uor_mods, "trait") do
		    if trait.id == "loyal" then
		        iloyal = "yes"
		    end
		 ...
		 if iloyal == "no" then
          ...
          wml.variables['total_on_call'] = ioc
